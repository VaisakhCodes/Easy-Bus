...
def adminmanageroutes():
	...
	if 'update' in request.form:
		...
		q="update routes set route_name='%s',splace_id='%s',eplace_id='%s' where route_id='%s'"%(route_name,starting_id,ending_id,pid)
		update(q)
		flash("updated successfully")
		return redirect(url_for('admin.adminmanageroutes'))
...
@admin.route('/registerbus', methods=['get','post'])
def registerbus():
	data={}
	...
	if "submit" in request.form:
		...
		lid=insert(q)
		q="insert into conductor values(null,'%s','%s','%s','%s','%s','%s')" %(lid,buss,fname,lname,age,phone)
		insert(q)

		q="insert into buslocation values(null,'%s','0','0',curdate())" %(buss)
		insert(q)
