...
//						s1.setAdapter(new ArrayAdapter<String>(getApplicationContext(),  R.layout.cust_list,splace));
//					}
//					else
//					{
...
//						}
//						//driver_list.setAdapter(new ArrayAdapter<String>(getApplicationContext(),R.layout.spin,ConfirmRide.name_s));
//						s2.setAdapter(new ArrayAdapter<String>(getApplicationContext(),  R.layout.cust_list,eplace));
//					}
//					else
//					{
//						Toast.makeText(getApplicationContext(), "No Data", Toast.LENGTH_LONG).show();
...
public class SearchRoutes extends Activity implements AdapterView.OnItemSelectedListener, AdapterView.OnItemClickListener, JsonResponse {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		s1 = findViewById(R.id.spsplace);
		s1.setOnItemSelectedListener(this);
		s2 = findViewById(R.id.speplace);
		s2.setOnItemSelectedListener(this);

		lv = findViewById(R.id.lvsroutes);
		lv.setOnItemClickListener(this);

		// Fetch start places initially
		fetchStartPlaces();
	}

	private void fetchStartPlaces() {
		JsonReq jr = new JsonReq();
		jr.json_response = (JsonResponse) SearchRoutes.this;
		String q = "viewspstart/";
		jr.execute(q);
	...
	private void fetchEndPlaces(String selectedStartPlaceId) {
		JsonReq jr = new JsonReq();
		jr.json_response = (JsonResponse) SearchRoutes.this;
		String q = "viewspend/?splace=" + selectedStartPlaceId;
		jr.execute(q);
	}

	private void fetchRoutes(String selectedStartPlaceId, String selectedEndPlaceId) {
		JsonReq jr = new JsonReq();
		jr.json_response = (JsonResponse) SearchRoutes.this;
		String q = "viewroutes/?splace=" + selectedStartPlaceId + "&eplace=" + selectedEndPlaceId;
		jr.execute(q);
	}
	...
	public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		if (arg0 == s1) {
			splaceids = splaceid[arg2];
			splaces = splace[arg2];
			// Fetch end places based on the selected start place
			fetchEndPlaces(splaceids);
		} else if (arg0 == s2) {
			eplaceids = eplaceid[arg2];
			eplaces = eplace[arg2];
			// Clear the list before fetching new routes
			lv.setAdapter(null);
			// Fetch routes based on the selected start and end places
			fetchRoutes(splaceids, eplaceids);
		}
	}
