...
def viewnearbystops():
	...
	q="SELECT *,(3959 * ACOS ( COS ( RADIANS('%s') ) * COS( RADIANS( latitude) ) * COS( RADIANS( longitude ) - RADIANS('%s') ) + SIN ( RADIANS('%s') ) * SIN( RADIANS( latitude ) ))) AS user_distance FROM places HAVING user_distance <30 ORDER BY user_distance ASC " %(lati,longi,lati)
	print(q)
	res=select(q)
	print(res)
	if res:
...
def viewnearbyplaces():
	...
	q="SELECT *,(3959 * ACOS ( COS ( RADIANS('%s') ) * COS( RADIANS( latitude) ) * COS( RADIANS( longitude ) - RADIANS('%s') ) + SIN ( RADIANS('%s') ) * SIN( RADIANS( latitude ) ))) AS user_distance FROM otherplaces HAVING user_distance <30 ORDER BY user_distance ASC " %(lati,longi,lati)
	res=select(q)
	if res:
		data['status']="success"
		data['data']=res
...
def viewroutes():
	...
	if res: 
		...
		res2=select(q)
		if res1[0]['stopnum']<res2[0]['stopnum']:
...
@api.route('/track_bus/',methods=['get','post'])
def track_bus():
	data={}
	data['method'] = 'track_bus'
	bus_id=request.args['bus_id']

	q="SELECT * FROM `buslocation` WHERE `bus_id`='%s'" %(bus_id)
	res=select(q)
	if res:
		data['status']="success"
		data['data']=res
...
@api.route('/send_emergency/',methods=['get','post'])
def send_emergency():
	data={}
	data['method'] = 'send_emergency'
	bus_id=request.args['bus_id']

	q="SELECT * FROM `conductor` WHERE `bus_id`='%s'" %(bus_id)
	res=select(q)
	if res:
		data['status']="success"
		data['data']=res
...
def notification():
    ...
    logid = request.args['conductor_id']
    q = "insert into notification values(null,(select conductor_id from conductor where log_id='%s'),'%s',curdate())" % (logid,notification)
    res=insert(q)
    if res:
        data['status'] = 'success'
    else:
...
# @api.route('/viewnenotification/',methods=['get','post'])
# def viewnenotification():
# 	data={}
	

# 	qry="SELECT * FROM reservations INNER JOIN trips USING(trip_id) where user_id='%s'"%()
# 	res1=select(qry)
# 	print(res1)
# 	if res1:
		
# 		bus_ids = [entry['bus_id'] for entry in res1]
# 		print(bus_ids)

# 		qry="select * from buses inner join conductor using(bus_id)"
# 		res2=select(qry)
# 		print("/////////////",res2)
# 		bu_id = [entry['bus_id'] for entry in res2]
# 		print(bu_id)
